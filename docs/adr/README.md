...
| [ADR-006](ADR-006-dast-and-load-testing.md) | DAST & Load Testing (ZAP + k6), Opt-In Scheduled | Accepted | Dynamic analysis (OWASP ZAP) and load testing (k6) run as opt-in, schedule/dispatch-driven reusable workflows — not PR gates — because they need a live target; findings surface as reports. |
